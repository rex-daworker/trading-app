...
function OverviewWidgets({ watchlist }: OverviewWidgetsProps) {
  ...
  const changes: Change[] = results
    .map((r, i) => (r.data ? { symbol: watchlist[i].symbol, dp: r.data.dp } : null))
    .filter((x): x is Change => x !== null)

  const tracked = watchlist.length
  const gainer = changes.length ? changes.reduce((a, b) => (b.dp > a.dp ? b : a)) : null
  const loser = changes.length ? changes.reduce((a, b) => (b.dp < a.dp ? b : a)) : null
  const avg = changes.length ? changes.reduce((s, c) => s + c.dp, 0) / changes.length : 0

  return (
    <div className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-4">
      <Widget label="Stocks tracked" value={String(tracked)} />
      <Widget
        label="Top gainer"
        value={gainer ? `${gainer.symbol}  ${gainer.dp >= 0 ? '+' : ''}${gainer.dp.toFixed(2)}%` : '—'}
